...
- Chạy bộ script kiểm thử tích hợp E2E giả lập hành vi người dùng bằng Python trên trình duyệt không đầu (ví dụ: `python scratch/test_e2e_curator.py`, `python scratch/test_e2e_curation.py` hoặc bộ test E2E tương ứng của US) chạy song song trên **cả 2 giao diện Desktop và Mobile** để kiểm thử độ tương thích responsive.
- **Lưu bằng chứng kiểm thử (E2E Test Evidence):** Đối với bất kỳ thay đổi nào tác động đến giao diện Web, AI bắt buộc phải chụp lại màn hình chạy test E2E thành công (bao gồm view Desktop và Mobile) và lưu tệp hình ảnh vào thư mục `docs/workflows/assets/` trước khi tiến hành cập nhật trạng thái User Story.
