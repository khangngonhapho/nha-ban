...
**So that** *tôi có thể dễ dàng quản lý mã nguồn, khôi phục khi cần thiết, và tích hợp các công cụ AI hỗ trợ viết tin đăng, viết tiêu đề ngắn dưới 85 ký tự, giải mã địa chỉ và chuẩn hóa tên đường một cách tập trung*  
...
- [x] Tích hợp giải pháp bảo mật **Script Properties** (Thuộc tính dự án) để tải động `OPENAI_API_KEY` tại runtime, tránh cứng hóa khóa API thực tế vào mã nguồn giúp ngăn chặn rò rỉ mã bảo mật trên Git.
- [x] Tài liệu hóa đầy đủ hướng dẫn cài đặt và các tính năng có trong custom script:
  ...
  - **Tự động viết Tiêu đề BDS (<85 ký tự)**: Gọi AI trích xuất thông số căn nhà (đường, phường, diện tích, kết cấu, giá...) từ dòng dữ liệu trên Source để sinh ra tiêu đề SEO chuyên nghiệp dưới 85 ký tự ghi trực tiếp vào cột AN.
...
### 🛡️ Hướng dẫn Cấu hình Bảo mật (Script Properties)
Để đảm bảo an toàn, không lộ API Key khi chia sẻ hoặc đẩy mã nguồn lên Git, mã nguồn đã được tích hợp cơ chế tự động đọc khóa từ **Script Properties** của Google Sheets.

**Các bước cấu hình:**
1. Trong giao diện Apps Script của Google Sheets, click chọn biểu tượng **Cài đặt dự án** (Project Settings - hình bánh răng ở menu dọc bên trái).
2. Cuộn xuống phần **Thuộc tính dự án** (Script Properties).
3. Click **Thêm thuộc tính dự án** (Add script property) và nhập:
   * **Thuộc tính (Property):** `OPENAI_API_KEY`
   * **Giá trị (Value):** Dán OpenAI API Key thực tế của bạn vào đây (ví dụ: `sk-proj-...`).
4. Click **Lưu thuộc tính** (Save script properties).

Mã nguồn sẽ tự động ưu tiên lấy API Key từ cấu hình này tại runtime. Nếu không tìm thấy thuộc tính dự án phù hợp, mã nguồn mới chuyển sang sử dụng biến `OPENAI_API_KEY` khai báo mặc định làm fallback.
