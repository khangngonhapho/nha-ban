...
## User story
**As an** Admin  
**I want** khi bấm nút Lưu trên Form Curation, hệ thống lưu thành công toàn bộ thông tin văn bản và thông số kỹ thuật đã điều chỉnh vào SQLite và Google Sheets Custom (Source 2).  
**So that** tôi có thể lưu vết các thay đổi biên tập về địa chỉ, diện tích, kết cấu và rộng hẻm tùy biến mà không ảnh hưởng đến dữ liệu cào thô gốc.

## Acceptance Criteria
- [x] **Lưu thông tin tùy biến về CSDL**:
  - Trích xuất dữ liệu từ các control văn bản trên Form: Tiêu đề public, Mô tả public, Giá public, Mã Khang Ngô, Hướng nhà, Tình trạng, Đánh giá, Số phòng ngủ, Số WC, Ngủ trệt, CHDV, và Rộng hẻm tùy chỉnh (từ control có sẵn).
  - Gửi yêu cầu cập nhật lên API `PUT /api/listings/<tk_id>`.
  - Cập nhật đúng các trường này vào SQLite bảng `listings_custom_v2` theo khóa ngoại `System_ID`.
- [x] **Đồng bộ Google Sheets Custom (Source 2)**:
  - Gọi Google Sheets API ghi đè dòng dữ liệu tương ứng trên tab `Custom` của file Source 2 với toàn bộ thông tin văn bản mới.
  - Tự động chèn cột `Custom_Rong_Hem` vào Google Sheets Custom nếu chưa có trong dòng tiêu đề 1.

---

## Solution
1. **Flask API `PUT /api/listings/<tk_id>` (`manager.py`):**
   - Đọc các trường text từ JSON payload.
   - Thêm cột `Custom_Rong_Hem` vào danh sách cập nhật động cho `listings_custom_v2`.
   - Thực hiện câu lệnh SQL `UPDATE` hoặc `INSERT` vào bảng `listings_custom_v2`.
2. **Frontend Curation Save (`lego_detail_admin.js`):**
   - Hàm `saveSourceChanges()` thu thập giá trị từ các trường nhập văn bản trên form.
   - Trích xuất giá trị rộng hẻm tùy biến từ control có sẵn và gửi lên API dưới dạng key `custom_rong_hem`.
3. **Google Sheets Sync (`pool_lego.py`):**
   - Hàm `sync_listing_to_pool2_sheets` build dòng dữ liệu tùy biến đầy đủ và đẩy lên Google Sheets Custom.

---

## 📝 Task Checklist (TODO)
- [x] **Backend Curation Save Handler**
  - [x] Cập nhật endpoint `PUT /api/listings/<tk_id>` lưu `Custom_Rong_Hem` và các trường văn bản tùy biến khác vào `listings_custom_v2`.
- [x] **Frontend Form Save Integration**
  - [x] Gửi payload chứa các thuộc tính văn bản tùy biến trong `saveSourceChanges()`.
- [x] **Google Sheets Custom Sync**
  - [x] Đẩy dữ liệu văn bản tùy biến đã biên tập lên tab `Custom` của Source 2 sheet.
