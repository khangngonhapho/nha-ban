...
## User story
**As an** Admin  
**I want** khi bấm nút "Lên sóng" (Active), hệ thống lọc các cột nghiệp vụ tùy biến đã được whitelist và xuất bản bản ghi sạch kèm mảng hình ảnh công khai rã phẳng lên Google Sheets Public.  
**So that** khách hàng truy cập trang chủ chỉ thấy thông tin sạch, chính xác và an toàn không bị lộ thông tin nhạy cảm.

## Acceptance Criteria
- [ ] **Whitelist cột xuất bản**:
  - Chỉ đồng bộ các trường đã được định nghĩa trong `PUBLIC_WHITELIST_HEADERS_BASE` (Tiêu đề, mô tả public, giá public, phường, quận, đường, số phòng ngủ, số WC, ngủ trệt, CHDV, và hẻm tùy chỉnh `Custom_Rong_Hem`).
  - Tuyệt đối không xuất bản địa chỉ thật, số nhà, note nội bộ hoặc thông tin liên hệ của chủ nhà/đầu chủ.
- [ ] **Rã phẳng mảng ảnh công khai**:
  - Rã chuỗi ảnh an toàn thành các cột rời rạc: Ảnh 1 đến Ảnh N (theo thứ tự sắp xếp của Admin).
  - Giữ nguyên ranh giới cột "Last updated" nằm trước toàn bộ danh sách các cột hình ảnh phẳng trên Sheet Public.
- [ ] **Publish Sync**: Đẩy thành công dòng dữ liệu public lên tab `Public` của file Public Sheet (`pool2_public_sheet_id`).

---

## Solution
1. **Google Sheets Public Sync (`pool_lego.py`):**
   - Hàm `sync_listing_to_pool2_sheets` đọc cấu hình `PUBLIC_WHITELIST_HEADERS_BASE`.
   - Trích xuất ảnh công khai từ `images_metadata_json` (loại bỏ ảnh nhạy cảm facade/diagram) và điền vào các cột Ảnh 1..Ảnh N sau cột "Last updated".
   - Thực hiện ghi hoặc thêm dòng mới lên worksheet `Public` của file Public.

---

## 📝 Task Checklist (TODO)
- [ ] **Public Whitelist Formatting**
  - [ ] Đảm bảo whitelist cột chứa `Custom_Rong_Hem` và lọc bỏ các cột PII.
  - [ ] Triển khai hàm rã ảnh phẳng công khai sau cột "Last updated" trong `pool_lego.py`.
- [ ] **Public Sheets Publish**
  - [ ] Thực hiện ghi đè/chèn dòng dữ liệu public sạch lên worksheet `Public` của Public Sheet khi trạng thái là Active.
