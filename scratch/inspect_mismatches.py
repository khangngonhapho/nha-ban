        ...
        if db_record:
            for sheet_col_idx, db_col_name in col_map.items():
                ...
                if sheet_val and "cloudinary.com" in sheet_val:
                    ...
                    is_match_r2 = db_val and "r2.dev" in db_val and file_base in db_val
                    is_skip_cld = db_val and "cloudinary.com" in db_val and db_val.strip() == sheet_val
                    is_empty = not db_val or not db_val.strip()
                    
                    if not is_match_r2 and not is_skip_cld and not is_empty:
                        print(f"Row {r_idx}, Col {sheet_col_idx+1} ({db_col_name}):")
                        print(f"  Sheet: {sheet_val}")
                        print(f"  DB:    {db_val}")
                        printed += 1
                        if printed >= 10:
                            return
