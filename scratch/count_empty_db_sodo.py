...
def main():
    ...
    temp_db = "D:/LHTBrain/01_PROJECTS/BDS-KhangNgo/raw_archive_temp_sodo.db"
    ...
    for r_idx, row in enumerate(all_rows[1:], start=2):
        ...
        for col_idx, db_col in col_map.items():
            if col_idx < len(row):
                sheet_val = row[col_idx].strip()
                if "cloudinary.com" in sheet_val:
                    total_cld_sheet += 1
                    if db_record:
                        db_val = db_record.get(db_col)
                        if not db_val or db_val.strip() == "":
                            empty_db_but_cld_sheet += 1
                    else:
                        empty_db_but_cld_sheet += 1
                        
    print(f"Total Cloudinary cells in Google Sheet: {total_cld_sheet}")
    print(f"Empty/Missing in SQLite but Cloudinary in Sheet: {empty_db_but_cld_sheet}")
