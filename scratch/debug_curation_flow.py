...
def main():
    ...
    with sync_playwright() as p:
        ...
        # Capture console error/log and alerts
        page.on("console", lambda msg: safe_print(f"CONSOLE: {msg.text}"))
        page.on("pageerror", lambda err: safe_print(f"PAGE ERROR: {err}"))
        page.on("dialog", lambda dialog: (safe_print(f"DIALOG ALERT TRIGGERED: {dialog.message}"), dialog.accept()))

        # Mock sheets requests
        def handle_admin_sheets(route):
            url = route.request.url
            method = route.request.method
            safe_print(f"Sheets Mock API: {method} {url}")
            if method == "GET":
                if "Source" in url:
                ...
                route.fulfill(content_type="application/json", body='{"spreadsheetId": "mock-sheet"}')
        ...
        # Print input values before toggling public image
        print("Values BEFORE toggling public image:")
        print("editPublicInteriorIndices:", page.evaluate("document.getElementById('editPublicInteriorIndices').value"))
        
        # Go to slide 2 and click public
        page.evaluate("gotoImageEditorSlide(2)")
        time.sleep(0.2)
        page.locator("#ctrlPublicBtn").click()
        time.sleep(0.2)
        
        # Print input values after toggling public image
        print("Values AFTER toggling public image:")
        print("editPublicInteriorIndices:", page.evaluate("document.getElementById('editPublicInteriorIndices').value"))
        
        # Open speed dial and save
        page.locator("#dialMainBtn").click()
        time.sleep(0.2)
        page.locator("#savePoolBtn").click()
