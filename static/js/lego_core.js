  ...
  showMaintenanceScreen() {
    document.body.innerHTML = `
      ...
      <style>
        ...
        #maint-icon {
          animation: maint-spin 4s linear infinite;
        }
      </style>
    `;
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.status === 'success' && data.config) {
          ...
          if (this.config.maintenance_mode === true && !this.isAdmin) {
      ...
      try {
        ...
        const fullList = sourceRows
          .map((sr, index) => {
            ...
            const p = {
              ...
              quan: sr[window.getSourceColumnIndex("quan", 9)] || '',
        ...
        poolDataRows.forEach((poolRow, prIdx) => {
          ...
          const p = {
            ...
            q: (isNaN(cleanQ) || cleanQ === '') ? cleanQ.toLowerCase() : 'q' + cleanQ,
            ql: cleanQ.toUpperCase(),
            quan: poolRow[window.getPoolColumnIndex("quan", 3)] || '',
