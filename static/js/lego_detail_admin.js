  ...
  // === renderImageEditorWidget ===
    function renderImageEditorWidget(p, forceNew = false) {
      ...
      if (window.imageEditorSortMode === 'display') {
        if (typeof window.sortSlidesByDisplayOrder === 'function') {
          window.sortSlidesByDisplayOrder(renderedCards, p);
        } else {
        ...
        <div class="admin-edit-group" id="imageEditorCurationGroup" style="margin-top: 10px;">
          ...
          <div class="image-editor-carousel-container">
            ...
            <div class="image-editor-control-panel">
              ...
              <div class="control-row" style="margin-top: 5px;">
                <button type="button" class="control-btn" onclick="window.activeImageSortByDisplay()" style="background: rgba(39, 174, 96, 0.15); border-color: #27ae60; color: #27ae60; width: ${!p.isFromPoolOnly ? '50%' : '100%'};">
                  🔀 Sắp xếp
                </button>
                ${!p.isFromPoolOnly ? `
                <button type="button" class="control-btn" onclick="window.uncheckAllCurationImages()" style="background: rgba(192, 57, 43, 0.15); border-color: var(--red); color: var(--red); width: 50%;">
                  ✕ Bỏ hết
                </button>
                ` : ''}
              </div>
    ...
    async function executePullFromPool(systemId, id, soNha, duong, btnElement) {
      ...
      try {
        ...
        if (existIdx !== -1) {
          ...
          // Thêm dòng mới - dùng append với range A3 để tránh Google Sheets API align sai cột (bỏ qua dòng 1, 2 trống)
          writeUrl = `https://sheets.googleapis.com/v4/spreadsheets/${SOURCE_SHEET_ID}/values/Source!A3:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`;
          writeMethod = 'POST';
        }
    ...
    window.saveSourceChanges = async function(id) {
      ...
      try {
        ...
        if (p.original_row_data) {
          ...
          const flatImages = [...cleanPublicImages];
          while (flatImages.length < 15) flatImages.push("");
          for (let i = 0; i < 10; i++) {
            p.original_row_data[20 + i] = flatImages[i];
          }
          for (let i = 10; i < 15; i++) {
            p.original_row_data[31 + i] = flatImages[i];
          }
          p.imgs = cleanPublicImages;
        }
