  ...
  // === reindexNaturalSequence ===
    window.reindexNaturalSequence = function(slides) {
      ...
      publicSlides.sort((a, b) => {
        ...
        // 1. Mặt Tiền luôn đứng ĐẦU TIÊN
        const aFacade = a.type === 'facade';
        const bFacade = b.type === 'facade';
        if (aFacade && !bFacade) return -9999;
        if (bFacade && !aFacade) return 9999;
        // 2. Ảnh nền (Cover) đứng thứ hai (sau Mặt Tiền)
        if (normCover && normA === normCover) return -9998;
        if (normCover && normB === normCover) return 9998;
